...
subtitle <-  'First 30 weeks of year'
wknumber <- 30

#import covid death file (this needs to be vetted in Excel before pulling in)
coviddeaths <-  read_csv('./data/covid_death_jul29.csv', col_types=cols(.default=col_character(), AGEYEARS=col_double(), DEATHDATE=col_date("%m/%d/%Y %H:%M:%S"))) %>% clean_names()
...
from deaths_main
where year(deathdate)>2014 and month(deathdate)<8
")
...
```{r}
deaths %>% group_by(yr, weeknumber) %>% summarise(count=n())
```
...
```{r}
accidents_YTD <- deaths %>% filter(weeknumber<=18, yr %in% c(2019, 2020), mannerdeath=='ACCIDENT') %>% group_by(yr, deathtype) %>% summarise(count=n())

accidents_YTD <-  accidents_YTD %>% ungroup() %>%  mutate(yr = as.character(yr))


ggplot(accidents_YTD) +
  # add bar for each discipline colored by gender
  geom_bar(aes(x = deathtype, y = count, fill = yr),
...
coviddeaths <-  coviddeaths %>% mutate(big_race_group = case_when(race=='White' ~'white',
                                                                  race=='Unknown' ~ 'unknown',
                                                                  TRUE ~ 'poc'),
                                       race_group = case_when(race=='White' ~'White',
                                                                  race=='Unknown' ~ 'Unknown',
                                                              race=='African American' ~ 'Black',
                                                              race=='American Indian'~'American Indian',
                                                              race %in% c("Other Asian", "Filipino", "Vietnamese", "Japanese", "Chinese", "Korean")~'Asian',
